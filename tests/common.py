...
class TestCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.specialty_cardiology = self.env['hr_hospital.specialty'].create({
            'name': 'Cardiology',
            'description': 'Specialized in heart diseases'
        })

        # Створення лікаря-ментора
        self.doctor_mentor = self.env['hr_hospital.doctor'].create({
            'first_name': 'Dmytro',
        ...
        self.intern_doctor = self.env['hr_hospital.doctor'].create({
            'first_name': 'Intern',
            'last_name': 'Ivanov',
            'specialty_id': self.specialty_cardiology.id,
            'is_intern': True,
        ...
        self.disease_type_infectious = (
            self.env['hr_hospital.disease.type'].create({
                'name': 'Infectious',
                'description': 'Infectious diseases'
            })
        )

        self.disease_flu = self.env['hr_hospital.disease'].create({
            'name': 'Flu',
            'disease_type_id': self.disease_type_infectious.id,
        })

        # Створення візиту пацієнта
        self.visit = self.env['hr_hospital.visit'].create({
        ...
        # Створення діагнозу
        self.diagnosis = self.env['hr_hospital.diagnosis'].create({
